...
const StreamsTable = ({ data, layout, onLayoutChange, sort, setSort, tableData, setTableData, topTableData, setTopTableData, bottomTableData, setBottomTableData }) => {
            ...
            {layout === 'grid' && (
                <div className="grid grid-cols-4  gap-4 min-h-0">
                    ...
                    {tableData.map((row) => (
                        ...
                        <StreamCard title={row.name} img="/Cup_PET.png" delivered={row.delivered} collected={row.collected} stream={row.stream} rate={row.rate} key={row.name}>
                        </StreamCard>
