...
const WarningTable = ({ data, layout, onLayoutChange, sort, setSort, tableData, setTableData, topTableData, setTopTableData, bottomTableData, setBottomTableData }) => {
                                ...
                                <tr className=' cursor-pointer' key={row.name}>
                                    {columns.map((col, i) => (
                                        <td onClick={() => handleRowClick(row)} onMouseOver={() => setHoveredRow(row.name)} onMouseOut={() => setHoveredRow(null)} className={i === columns.length - 1 ? ' hover:animate-pulse select-none text-left h-8 p-0 m-0 leading-7' : 'hover:text-grey select-none text-left border-r-2 border-grey h-8 p-0 m-0 leading-7'} key={i}>
                                            <div className={row.Status == 'Pending' ? i === 0 ? 'bg-accent text-white text-left rounded-l-full h-7 pl-4 m-0 leading-7 items-center mt-1 transition-all duration-300' : i === columns.length - 1 ? 'bg-accent text-white text-left rounded-r-full h-7 pl-4 m-0 leading-7 items-center mt-1 transition-all duration-300' : 'bg-accent text-white text-left h-7 pl-4 m-0 leading-7 items-center mt-1 transition-all duration-300' : i === 0 ? 'bg-white text-left rounded-l-full h-7 pl-4 m-0 leading-7 items-center mt-1 transition-all duration-300' : i === columns.length - 1 ? 'bg-white text-left rounded-r-full h-7 pl-4 m-0 leading-7 items-center mt-1 transition-all duration-300' : 'bg-white text-left h-7 pl-4 m-0 leading-7 items-center mt-1 transition-all duration-300'}>{row[col]}</div>
                                        </td>
                                    ))}
            ...
            {showModal && (
                <div className='absolute modal left-0 top-0 z-50'>
                    <div className=' fixed grid place-content-center inset-0 z-50'>
                        <div className='w-[700px]  inset-0 z-50'>
                            <WarningCard {...selectedData} isOpen={showModal} setIsOpen={setShowModal} />
                        </div>
